document.addEventListener('DOMContentLoaded', function() {
    ...
    var wmsLayers = [
        { url: 'https://geoserver.scrd.gov.co/geoserver/Investigacion_Cultured_Maps/wms', layerName: 'Investigacion_Cultured_Maps:Subindice_de_Ambiente_2021', displayName: 'Subindice_Ambiente_2021' },
        { url: 'https://geoserver.scrd.gov.co/geoserver/Investigacion_Cultured_Maps/wms', layerName: 'Investigacion_Cultured_Maps:Subindice_de_Ambiente_2023', displayName: 'Subindice_Ambiente_2023' }
    ];

    var overlays = {}, activeLayers = [];

    // Crear las capas iniciales con opacidad ajustable
    var layer2021 = L.tileLayer.wms(wmsLayers[0].url, {
        layers: wmsLayers[0].layerName,
        format: 'image/png',
        transparent: true,
        maxZoom: 21,
        opacity: 1 // Capa inicial completamente visible
    }).addTo(map);

    var layer2023 = L.tileLayer.wms(wmsLayers[1].url, {
        layers: wmsLayers[1].layerName,
        format: 'image/png',
        transparent: true,
        maxZoom: 21,
        opacity: 0 // Capa inicial oculta
    }).addTo(map);

    overlays[wmsLayers[0].displayName] = layer2021;
    overlays[wmsLayers[1].displayName] = layer2023;

    // Agregar control de capas al mapa
    L.control.layers(null, overlays, { collapsed: false }).addTo(map);

    // Control deslizante para ajustar las opacidades
    var slider = document.getElementById('layer-slider');
    slider.addEventListener('input', function () {
        var value = parseFloat(slider.value);
        layer2021.setOpacity(1 - value); // Reducir visibilidad de la capa 2021
        layer2023.setOpacity(value);     // Incrementar visibilidad de la capa 2023
    });
